...
dist:
	...
	tar -zcvf $(DIST)/helm-template-linux-$(VERSION).tgz tpl README.md LICENSE plugin.yaml
	GOOS=darwin GOARCH=amd64 go build -o tpl -ldflags $(LDFLAGS) ./main.go
	tar -zcvf $(DIST)/helm-template-macos-$(VERSION).tgz tpl README.md LICENSE plugin.yaml
